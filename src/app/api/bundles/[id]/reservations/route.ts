...
export async function POST(
  ...
  const { customer_name, customer_email, customer_phone, start_date, end_date, persons, bundle_title } = body;
  ...
  const { data, error } = await supabase
    ...
    .insert({
      bundle_id: (isNaN(bundleId) || bundleId === 0) ? null : bundleId,
      bundle_title: bundle_title || null,
      customer_name,
      customer_email,
